...
export function allocatorMatches(
  ...
  return organizations.flatMap((org) => {
    ...
    const prospect: IndexProspect = {
      ...
      whyFit: `${category?.label || "Allocator"}: ${fund.name} has an evidenced ${fund.mandate} mandate${match.overlaps.length ? ` with overlap in ${match.overlaps.slice(0, 4).join(", ")}` : ""}.`,
      ...
      evidence: evidence.map((item) => ({ label: `${item.sourceName}: ${item.title}`, url: item.url, snippet: item.excerpt })),
